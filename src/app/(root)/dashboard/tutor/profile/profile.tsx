...
export default function Profile() {
  ...
  const badges = [
    {
      text: user.user?.grades?.[0] ?? "—",
      color: "bg-[#7c4cff26] text-[#7c4cff]",
    },
    { text: "Age 4-12", color: "bg-[#4c76ff26] text-[#4c76ff]" },
    {
      text: `${
        Array.isArray(user.user?.subjects) ? user.user!.subjects.length : 0
      } subjects`,
      color: "bg-[#ff9d4c26] text-[#ff9d4c]",
    },
    { text: user.user?.timezone ?? "—", color: "bg-[#c6f6d5] text-[#038536]" },
  ];

  const availability =
    user.user.role === Role.TEACHER
      ? (user.user as Teacher).availability ?? []
      : [];

  // Normalize availability to an array for getScheduleString
  const availabilityArray: Availability[] = Array.isArray(availability)
    ? (availability as Availability[])
    : availability
    ? recordToAvailability(availability as Record<string, string[]>)
    : [];
  ...
  return (
    <div className="flex flex-col gap-8 items-center py-10">
      ...
      <div className="flex flex-col md:flex-row items-center gap-10 w-full max-w-6xl">
        ...
        <div className="bg-[#f4f4f4] p-6 md:p-8 rounded-xl shadow-sm w-full">
          <div className="flex justify-between items-center mb-6">
            <span className="bg-[#031d95] text-white text-sm px-4 py-1 rounded-md font-medium capitalize">
              {user.user.role}
            </span>
      ...
      <AboutTutor tutor={user.user as Teacher} />
      <MyQualificationsSection tutor={user.user as Teacher} />
    </div>
  );
}
